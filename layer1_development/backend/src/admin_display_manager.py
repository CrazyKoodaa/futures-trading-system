...
class DisplayManager:
    ...
    def __init__(self, console: Console, tui_components: TUIComponents, status: SystemStatus):
    ...
    def _setup_layout(self) -> None:
        ...
        self.layout.split(
            Layout(name="header", size=5),  # Increased from 3 to 5 to show header properly
            Layout(name="body"),
            Layout(name="footer", size=12)  # Increased from 10 to 12 for better status display
        )
        ...
        self.layout["content"].split(
            Layout(name="main_content", ratio=3),
            Layout(name="results", ratio=2)
        )
    
    def render_layout(self, selected_menu_index: int = 0, results_content: str = "") -> Layout:
        ...
        if hasattr(self.status, 'last_operation_result') and self.status.last_operation_result:
    ...
    def render_header(self) -> Panel:
        ...
        connection_status = "● Connected" if self.status.rithmic_connected else "○ Disconnected"
        status_color = "green" if self.status.rithmic_connected else "red"
        header_content.append(connection_status, style=status_color)
        
        # Add database status
        header_content.append(" | ")
        db_status = "● DB Connected" if self.status.database_connected else "○ DB Disconnected"
    ...
    def create_progress_table(self, download_progress: Dict[str, Any]) -> Table:
        ...
        for loop_idx, (symbol, progress) in enumerate(download_progress.items()):
            symbol_idx = loop_idx  # Local copy
            if hasattr(progress, 'current_step'):
                ...
                step = "Downloading"
                percent = f"{progress * 100:.1f}%" if isinstance(progress, float) else "0%"
                status = "🔄 In Progress"
    ...
    def create_symbol_table(self, symbols_data: List[Union[Dict[str, Any], Any]]) -> Table:
        ...
        """
        table = Table(title="Available Symbols", show_header=True, header_style="bold cyan")
        ...
        for loop_idx, symbol_data in enumerate(symbols_data):
            data_idx = loop_idx  # Local copy to avoid cell variable issue
    ...
    def create_connection_status_panel(self) -> Panel:
        ...
        modules = [
            ("Connection", "admin_rithmic_connection.py", self.status.rithmic_connected),
            ("Symbols", "admin_rithmic_symbols.py", bool(self.status.symbol_search_results)),
            ("Historical", "admin_rithmic_historical.py", bool(self.status.download_progress)),
            ("Operations", "admin_rithmic_operations.py", self.status.database_connected)
        ]
        
        for idx2, (module_name, module_file, is_active) in enumerate(modules):
    ...
    def create_module_status_tree(self) -> Tree:
        ...
        # Connection Module
        connection_branch = tree.add("🔗 Connection Module (admin_rithmic_connection.py)")
        if self.status.rithmic_connected:
        ...
        if self.status.symbol_search_results:
            symbols_branch.add(f"✅ Symbol Search: {len(self.status.symbol_search_results)} results")
        else:
            symbols_branch.add("❌ Symbol Search: No results")
        
        # Historical Module
        historical_branch = tree.add("📊 Historical Module (admin_rithmic_historical.py)")
        if self.status.download_progress:
            historical_branch.add(f"✅ Downloads: {len(self.status.download_progress)} active")
            # Avoid cell variable issue
            progress_items = list(self.status.download_progress.items())
            for symbol, progress in progress_items:
                if hasattr(progress, 'current_step'):
                    ...
                    if callable(completion_pct):
                        try:
                            ...
                            percentage_val = pct_val * 100 if isinstance(pct_val, (int, float)) else 0
                        except:
                            percentage_val = 0
                        ...
                        percentage_val = 0
                    historical_branch.add(f"📈 {symbol}: {progress.current_step} ({percentage_val:.1f}%)")
        else:
            historical_branch.add("❌ Downloads: None active")
        
        # Operations Module
        operations_branch = tree.add("🗃️ Operations Module (admin_rithmic_operations.py)")
        if self.status.database_connected:
    ...
    def set_operation_result(self, result: Union[Dict[str, Any], str, List, None, Any]) -> None:
