...
class RithmicAdminTUI:
    ...
    def _build_connection_test_results(
        self,
        db_success: bool,
        db_message: str,
        rithmic_success: bool,
        rithmic_message: str,
        ...
        if rithmic_message:
            # Clean up the message and format it properly
            clean_rithmic_msg = (
                rithmic_message.replace("❌", "").replace("✅", "").strip()
            )
            if rithmic_success:
    ...
    async def handle_menu_selection(self, index: int):
        ...
        try:
            ...
            if operation_key == "test_connections":
                ...
                # First connect to Rithmic
                self._update_status("Connecting to Rithmic...", "info")
                rithmic_connect_success, rithmic_connect_message = await self.connection_manager.connect()

                # Test Rithmic connection
                self._update_status("Testing Rithmic connection...", "info")
                rithmic_success, rithmic_message = (
                ...
                result = {
                    ...
                    "details": {
                        ...
                        "rithmic": {
                            "success": rithmic_success,
                            "message": rithmic_message,
                        },
                    },
                }
                ...
                result = {
                    "status": "error",
                    "message": f"Unknown operation: {operation_key}",
                }
            ...
            if isinstance(result, dict):
                ...
                self.status.last_operation_result = (
                    str(result) if result else "Operation completed"
                )
        ...
        for manager_name, manager in cleanup_managers:
            if hasattr(manager, "cleanup"):
                try:
                    ...
                    self._update_status(
                        f"Error during {manager_name} cleanup: {str(e)}", "error"
                    )

        # Stop keyboard handling
        if self.keyboard_handler == "keyboard" and KEYBOARD_AVAILABLE:
            try:
                import keyboard
                ...
                self._update_status(
                    f"Error stopping keyboard handler: {str(e)}", "error"
                )
    ...
    async def run(self):
        ...
        try:
            ...
            self._update_status(
                "Welcome to Rithmic Admin Tool! Use ↑/↓ or j/k to navigate, Enter to select.",
                "info",
            )
    ...
    async def handle_fallback_input(self):
        ...
        self._update_status(
            "No keyboard handler available. Use 'q' to quit or run with proper terminal.",
            "warning",
        )
