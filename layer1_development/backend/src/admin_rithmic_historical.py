...
try:
    from async_rithmic import TimeBarType as RithmicTimeBarType
    TimeBarType = RithmicTimeBarType
except ImportError:
...
class RithmicHistoricalManager:
        ...
        for bar in raw_bars:
            try:
                ...
                if timestamp is None:
                    timestamp = bar.get("timestamp") or datetime.now()
    ...
    async def cleanup(self) -> None:
        """
        Clean up resources used by the historical manager.
        
        This method ensures all resources are properly released when the manager
        is no longer needed.
        """
        logger.info("Cleaning up RithmicHistoricalManager resources")
        # Cancel any pending tasks (if they exist)
        if hasattr(self, 'active_tasks'):
            for task in self.active_tasks:
                if not task.done():
                    task.cancel()
                
        # Clear cached data (if cache exists)
        if hasattr(self, 'cache'):
            self.cache.clear()
        logger.info("RithmicHistoricalManager cleanup completed")
