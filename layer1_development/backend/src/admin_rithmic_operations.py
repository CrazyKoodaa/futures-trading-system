...
class RithmicOperationsManager:
    ...
    def __init__(self, progress_callback: Callable = None):
        ...
        self.symbol_manager = RithmicSymbolManager(self.connection_manager, progress_callback)
        self.historical_manager = RithmicHistoricalManager(
            self.connection_manager, None, progress_callback  # database_ops injected later
        )
    ...
    async def search_and_check_symbols(self, search_term: str, exchange: str = "CME") -> str:
        """
        ...
        """
        logger.info(f"Starting symbol search and check for '{search_term}' on {exchange}")
        ...
        try:
            ...
            if selected_contracts:
                ...
                verification_results = await self.verify_contracts_with_database(selected_contracts)
            else:
                verification_results = {}
            
            # 5. Return formatted results
            self._update_progress("Formatting results...")
            formatted_results = self.format_symbol_search_results(results, verification_results)
    ...
    async def simulate_contract_selection(self, search_results: List[Dict]) -> List[str]:
        """
        ...
        try:
            ...
            sorted_contracts = sorted(filtered, key=lambda x: x.get('expiration_date', ''))
    ...
    async def verify_contracts_with_database(self, contracts: List[str]) -> Dict[str, Dict]:
        """
        ...
        if not hasattr(self, 'database_ops') or not self.database_ops:
            logger.warning("Database operations not available for contract verification")
            return {}
    ...
    def format_symbol_search_results(self, search_results: List[Dict], 
                                   db_verification: Dict[str, Dict]) -> str:
        """
        ...
        for result in search_results:
            ...
            markdown_result += f"- **Product**: {result.get('product_code', 'Unknown')}\n"
            markdown_result += f"- **Expiration**: {result.get('expiration_date', 'Unknown')}\n"
            ...
            if symbol in db_verification:
                ...
                if 'error' not in db_info:
                    markdown_result += f"- **Database Records**: {db_info.get('total_records', 0):,}\n"
                    if 'latest_data' in db_info:
                        markdown_result += f"- **Latest Data**: {db_info['latest_data']}\n"
                    if 'date_range' in db_info:
    ...
    async def download_historical_data(self, contracts: List[str], days: int = 7) -> str:
        """
        ...
        try:
            ...
            if not prerequisites_check['valid']:
                logger.error(f"Prerequisites validation failed: {prerequisites_check['message']}")
            ...
            self._update_progress(f"Downloading {days} days of data for {len(contracts)} contracts...")
    ...
    async def validate_download_prerequisites(self, contracts: List[str]) -> Dict[str, Any]:
        """
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'connection': {
                ...
                'status': 'Connected' if self.connection_manager.is_connected else 'Disconnected'
            },
            'database': {
                'available': hasattr(self, 'database_ops') and self.database_ops is not None
            },
            'managers': {
    ...
    async def cleanup(self):
        ...
        logger.info(f"Rithmic operations cleanup completed: {'; '.join(cleanup_results)}")
...
async def create_rithmic_operations(progress_callback: Callable = None) -> RithmicOperationsManager:
    """
