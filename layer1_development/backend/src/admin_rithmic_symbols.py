...
from typing import Dict, List, Optional, Callable, Tuple, Any, Protocol
from dataclasses import dataclass
from enum import Enum


# Define our own InstrumentType to avoid import conflicts
class InstrumentType(Enum):
    FUTURE = "FUTURE"


# Import the Rithmic connection manager
try:
    from async_rithmic import InstrumentType as RithmicInstrumentType

    USING_RITHMIC_LIB = True
except ImportError:
    # Fallback for testing
    RithmicInstrumentType = InstrumentType
    USING_RITHMIC_LIB = False


# Get the appropriate instrument type for the Rithmic API
def get_rithmic_future_type():
    """Return the appropriate FUTURE instrument type for the Rithmic API"""
    if USING_RITHMIC_LIB:
        return RithmicInstrumentType.FUTURE
    else:
...
class RithmicSymbolManager:
    ...
    def _report_progress(self, message: str, percentage: Optional[int] = None):
        """Report progress if callback is available"""
        if self.progress_callback:
            # Convert None to 0 if percentage is None
            self.progress_callback(message, 0 if percentage is None else percentage)
    ...
    async def process_wildcard_search(
        self, search_term: str, exchange: str
    ) -> List[Dict]:
        """
    ...
    async def _direct_symbol_search(self, symbol: str, exchange: str) -> List[Dict]:
        ...
        try:
            ...
            if client is None:
                raise ConnectionError("Rithmic client is not available")
            ...
            search_results = await client.search_symbols(
                ...
                exchange=exchange,
                instrument_type=(
                    None if not USING_RITHMIC_LIB else get_rithmic_future_type()
                ),
            )
        ...
        try:
            ...
            return {
                ...
                "month_name": MONTH_NAMES.get(
                    MONTH_CODES.get(contract_month, 0), "Unknown"
                ),
    ...
    def filter_quarterly_contracts(self, contracts: List[Dict]) -> List[Dict]:
        ...
        self.logger.info(
            f"Filtered {len(contracts)} contracts to {len(filtered)} quarterly contracts"
        )
        return filtered

    async def get_front_month_contract(
        self, symbol: str, exchange: str = "CME"
    ) -> Optional[str]:
        """
    ...
    async def validate_contracts(
        self, contracts: List[str], exchange: str = "CME"
    ) -> Dict[str, bool]:
        """
        ...
        try:
            ...
            for i, contract in enumerate(contracts):
                try:
                    ...
                    validation_results[contract] = details is not None and details.get(
                        "is_active", False
                    )
                    ...
                    self._report_progress(
                        f"Validated {i + 1}/{len(contracts)} contracts", progress
                    )
            ...
            self._report_progress(
                f"Validation complete: {valid_count}/{len(contracts)} contracts valid"
            )
    ...
    async def get_contract_details(
        self, contract: str, exchange: str = "CME"
    ) -> Optional[Dict]:
        """
        ...
        try:
            ...
            # Get contract details from API - use search_symbols as a workaround
            # since get_contract_details is not available in the RithmicClient
            search_results = await client.search_symbols(
                search_text=contract, exchange=exchange, instrument_type=None
            )

            # Find the exact contract match
            details = next(
                (r for r in search_results if r.get("symbol") == contract), None
            )
    ...
    async def check_database_data(self, contracts: List[str]) -> Dict[str, Dict]:
        ...
        try:
            self._report_progress(
                f"Checking database data for {len(contracts)} contracts"
            )
    ...
    def format_contracts_for_display(self, contracts: List[Dict]) -> str:
        ...
        for base_symbol, contract_list in grouped.items():
            ...
            for contract in sorted_contracts:
                ...
                status = (
                    "✅ Active" if contract.get("is_active", True) else "❌ Expired"
                )
                ...
                mock_results.append(
                    {
                        ...
                        "contract_year": year,
                        "month_name": MONTH_NAMES.get(
                            MONTH_CODES.get(month, 0), "Unknown"
                        ),
    ...
    def _get_mock_contract_details(
        self, contract: str, exchange: str
    ) -> Optional[Dict]:
...
def parse_contract_symbol(symbol: str) -> Dict[str, Any]:
    ...
    # Create a minimal mock connection manager for parsing
    # Use Protocol to make type checker happy
    class RithmicConnectionManagerProtocol(Protocol):
        is_connected: bool

        @property
        def client(self): ...

    class MinimalMockManager:
        def __init__(self):
    ...
    # Type ignore to bypass the type check since we're just using this for parsing
    manager = RithmicSymbolManager(MinimalMockManager())  # type: ignore
...
if __name__ == "__main__":
    ...
    async def test_symbol_manager():
        ...
        class MockConnectionManager:
            ...
            async def search_symbols(self, search_text, exchange, instrument_type):
                return []

            async def get_contract_details(self, contract, exchange):
                return None

            # Additional methods to match RithmicConnectionManager interface
        ...
        # Type ignore to bypass the type check since we're just using this for testing
        manager = RithmicSymbolManager(mock_conn)  # type: ignore
