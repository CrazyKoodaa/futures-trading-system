...
from async_rithmic import (
    RithmicClient, Gateway, ReconnectionSettings, RetrySettings
)
...
class RithmicConnectionManager:
    ...
    async def connect(self) -> Tuple[bool, str]:
        ...
        async with self._lock:
            ...
            try:
                self._report_progress("🔄 **Initializing connection to Rithmic API...**")
                ...
                self._report_progress("⚙️ Configuring reconnection settings...")
                ...
                try:
                    self.client = RithmicClient(
                        ...
                        system_name=self.connection_config.get('system_name', 'AdminTool'),
                        app_name=self.connection_config.get('app_name', 'RithmicAdmin'),
                ...
                try:
                    ...
                    error_msg = "❌ **Connection Timeout**: Failed to connect within 30 seconds"
                    self.connection_errors.append({
                        'timestamp': datetime.now(),
                        'error': error_msg,
                        'type': 'timeout'
                ...
                success_msg = (
                    ...
                    f"🌐 **Gateway**: Chicago\n"
                    f"👤 **User**: {self.connection_config['username']}\n"
        ...
        try:
            ...
            if self.last_heartbeat:
                ...
                if heartbeat_age > 60:  # 1 minute threshold
                    return False, f"⚠️ **Stale connection**: Last heartbeat {heartbeat_age:.0f}s ago"
            
            # Check connection age
            if self.connection_start_time:
                connection_age = (datetime.now() - self.connection_start_time).total_seconds()
                if connection_age > self.max_connection_age:
                    return False, f"⚠️ **Connection aged**: {connection_age/3600:.1f}h old"
    ...
    def get_connection_status(self) -> Dict[str, Any]:
        ...
        if self.connection_start_time:
            status['uptime_seconds'] = (datetime.now() - self.connection_start_time).total_seconds()
    ...
    async def get_connection_info(self) -> Dict[str, Any]:
        ...
        if self.client and self.is_connected:
            try:
                ...
                info['server'] = {
                    'name': self.connection_config.get('server_name', 'Unknown'),
                    'environment': self.connection_config.get('environment', 'Unknown'),
                ...
                info['user'] = {
                    ...
                    'account': self.connection_config.get('account', 'Unknown')
                }
            except Exception as e:
...
def get_connection_manager(progress_callback: Callable = None) -> RithmicConnectionManager:
    """
...
async def ensure_connection(progress_callback: Callable = None) -> Tuple[bool, str, RithmicConnectionManager]:
    """
