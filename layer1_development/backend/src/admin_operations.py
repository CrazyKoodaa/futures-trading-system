...
class AdminOperations:
    ...
    def set_operations(self, db_ops, connection_manager: RithmicConnectionManager, 
                      symbol_ops: RithmicSymbolManager, historical_ops: RithmicHistoricalManager,
    ...
    async def test_connections(self) -> str:
        ...
        try:
            ...
            if self.status.db_connected and self.status.rithmic_connected:
                ...
                result += "⚠️ **Partial connectivity** - Some operations may be limited.\n"
            else:
    ...
    async def search_and_check_symbols(self, search_term: str = "", exchange: str = "CME") -> str:
        ...
        try:
            ...
            if not raw_results:
                return result + f"No symbols found matching '{search_term}' on {exchange}\n"
            ...
            if has_wildcards:
                ...
                for raw_result in raw_results:
                    ...
                    if (re.match(pattern, symbol, re.IGNORECASE) or 
                        re.match(pattern, raw_result.get('product_code', ''), re.IGNORECASE)):
            ...
            selected_results = await self._simulate_interactive_selection(search_results)
    ...
    async def download_historical_data(self, days: int = 7, download_seconds: bool = True, download_minutes: bool = False) -> str:
        """
        ...
        if not self.status.available_contracts:
            return "❌ **Error**: No contracts available. Please search symbols first.\n"
        ...
        total_operations = len(self.status.available_contracts) * (int(download_seconds) + int(download_minutes))
        current_operation = 0
        
        self.download_progress = {symbol: DownloadProgress() for symbol in self.status.available_contracts.keys()}
            ...
            async with get_async_session() as session:
                ...
                for symbol, contract_info in self.status.available_contracts.items():
                    ...
                    if download_seconds:
                        ...
                        self.progress_callback(f"Downloading second bars for {symbol}...", progress)
                    ...
                    if download_minutes:
                        current_operation += 1
                        progress = current_operation / total_operations
                        self.progress_callback(f"Downloading minute bars for {symbol}...", progress)
                ...
                try:
                    ...
                    if recent_data:
                        ...
                        for row in recent_data:
                            ohlc = f"{row[4]:.2f}/{row[5]:.2f}/{row[6]:.2f}/{row[7]:.2f}"
    ...
    async def _simulate_interactive_selection(self, search_results: List[Dict]) -> List[Dict]:
        """
    ...
    async def _check_database_data(self, symbol: str, exchange: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not seconds_info.get('exists', False) or not minutes_info.get('exists', False):
                return {
                    'exists': False,
                    'seconds_data': 0,
                    'minutes_data': 0,
    ...
    async def _download_second_bars(self, symbol: str, exchange: str, days: int) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self._report_progress(f"Downloading {days} days of second bars for {symbol}...", 0.4)
            ...
            if self.db_ops and 'bars' in data and data['bars']:
                self._report_progress(f"Inserting {len(data['bars'])} second bars into database...", 0.6)
    ...
    async def _download_minute_bars(self, symbol: str, exchange: str, days: int) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Request historical data
            self._report_progress(f"Downloading {days} days of minute bars for {symbol}...", 0.7)
            ...
            # Insert data into database
            if self.db_ops and 'bars' in data and data['bars']:
                self._report_progress(f"Inserting {len(data['bars'])} minute bars into database...", 0.9)
    ...
    async def initialize_database(self) -> str:
        ...
        if not self.db_ops:
            return result + "❌ **Error**: Database operations manager not initialized.\n\nPlease check your database configuration and try again."
        ...
        try:
            ...
            if success:
                ...
                if connection_info and connection_info.get('connected', False):
                    ...
                    hypertables = connection_info.get('database_info', {}).get('hypertables', [])
                    if hypertables:
                        result += "\n### Hypertables\n\n"
