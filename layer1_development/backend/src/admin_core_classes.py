...
class DownloadProgress:
    ...
    current_step: int = 0
    # Removed completion_percentage attribute as it conflicts with the property method
    error: str = ""
    progress: float = 0.0
    ...
    def update_chunk(self, increment: int = 1) -> None:
        ...
        self.completed_chunks = min(
            self.completed_chunks + increment, self.total_chunks
        )
...
class SystemStatus:
    ...
    active_operations: Dict[str, str] = field(
...
class HistoricalDataRequest:
    ...
    timeframes: List[str] = field(
        default_factory=lambda: ["1s", "1m", "5m", "15m", "1h", "1d"]
    )
...
class TUIComponents:
    ...
    def create_header_panel(self, **_unused_kwargs) -> Panel:
        ...
        rithmic_status = Text(
            ...
            style=(
                ColorScheme.SUCCESS
                if self.system_status.rithmic_connected
        ...
        db_status = Text(
            ...
            style=(
                ColorScheme.SUCCESS
                if self.system_status.database_connected
                else ColorScheme.ERROR
            ),
        )
        ...
        header_content = Group(
            Text(
                "RITHMIC FUTURES TRADING ADMIN TOOL",
                style=ColorScheme.HEADER,
                justify="center",
            ),
            ...
            Columns(
                [
                    Text(
                        f"Rithmic {rithmic_status.plain} {'Connected' if self.system_status.rithmic_connected else 'Disconnected'}",
                        style=(
                            ColorScheme.SUCCESS
                            if self.system_status.rithmic_connected
                            else ColorScheme.ERROR
                    ...
                    Text(
                        f"Database {db_status.plain} {'Connected' if self.system_status.database_connected else 'Disconnected'}",
                        style=(
                            ColorScheme.SUCCESS
                            if self.system_status.database_connected
                            else ColorScheme.ERROR
                    ...
                    Text(
                        f"Session: {self._format_time_duration(self.system_status.session_duration_time)}",
    ...
    def create_menu_panel(self, selected_index: int = 0) -> Panel:
        ...
        for i, (item, description) in enumerate(zip(MENU_ITEMS, MENU_DESCRIPTIONS)):
            ...
            if i == selected_index:
                ...
                menu_items.append(Text(f"   {description}", style="dim white"))
    ...
    def create_status_panel(self) -> Panel:
        ...
        status_table.add_row(
            "Exchange:",
            Text(self.system_status.current_exchange, style=ColorScheme.EXCHANGE),
        )

        if self.system_status.current_symbols:
            symbols_text = ", ".join(self.system_status.current_symbols)
            status_table.add_row(
                "Symbols:", Text(symbols_text, style=ColorScheme.SYMBOL)
            )
        else:
        ...
        if self.system_status.available_contracts:
            ...
            status_table.add_row(
                "Contracts:",
                Text(" | ".join(contracts_info), style=ColorScheme.CONTRACT),
            )
        ...
        if self.system_status.last_operation_result:
            status_table.add_row(
                "Last Result:",
                Text(self.system_status.last_operation_result, style=ColorScheme.INFO),
            )
        ...
        if self.system_status.last_error:
            status_table.add_row(
                "Last Error:",
                Text(self.system_status.last_error, style=ColorScheme.ERROR),
            )
    ...
    def create_results_panel(self, content: str = "", title: str = "Results") -> Panel:
        ...
        result_content: Union[Markdown, Text]
        try:
    ...
    def create_progress_panel(self) -> Panel:
        ...
        for symbol, progress in self.system_status.download_progress.items():
            ...
            if progress.current_operation:
                operation_text = Text(
                    f"{symbol}: {progress.current_operation}", style=ColorScheme.INFO
                )
            ...
            if progress.current_timeframe:
                timeframe_text = Text(
                    f"  Timeframe: {progress.current_timeframe}", style="dim white"
                )
            ...
            if progress.records_processed > 0:
                stats_text = Text(
                    f"  Records: {progress.records_processed:,}", style="dim white"
                )
            ...
            if progress.error_count > 0:
                error_text = Text(
                    f"  Errors: {progress.error_count}", style=ColorScheme.WARNING
                )
    ...
    def create_connection_status_panel(
        self, connection_info: Optional[RithmicConnectionInfo] = None
    ) -> Panel:
        ...
        conn_table.add_row(
            "Gateway:", Text(connection_info.gateway, style=ColorScheme.INFO)
        )
        conn_table.add_row(
            "Server:", Text(connection_info.server_name, style=ColorScheme.INFO)
        )
        conn_table.add_row(
            "User:", Text(connection_info.user, style=ColorScheme.SYMBOL)
        )
        conn_table.add_row(
            "App:",
            Text(
                f"{connection_info.app_name} v{connection_info.app_version}",
                style="white",
            ),
        )
        ...
        if connection_info.connection_time:
            ...
            conn_table.add_row(
                "Connected:",
                Text(self._format_time_duration(duration), style=ColorScheme.INFO),
            )
        ...
        conn_table.add_row(
            ...
            Text(
                ...
                style=(
                    ColorScheme.SUCCESS
                    if connection_info.connection_stable
        ...
        if connection_info.error_count > 0:
            conn_table.add_row(
                "Errors:",
                Text(str(connection_info.error_count), style=ColorScheme.ERROR),
            )
    ...
    def create_symbol_search_panel(
        self, search_results: List[SymbolSearchResult]
    ) -> Panel:
        ...
        return Panel(
            Group(
                results_table, Text(footer_text, style="dim white", justify="center")
            ),
            title="Symbol Search Results",
            border_style=ColorScheme.PANEL_BORDER,
            padding=(1, 1),
    ...
    def create_operations_panel(self) -> Panel:
        ...
        for op_id, status in self.system_status.active_operations.items():
            ...
            status_style = (
                ...
                else ColorScheme.INFO
            )
            if "error" in status.lower() or "failed" in status.lower():
                status_style = ColorScheme.ERROR

            ops_table.add_row(operation, Text(status, style=status_style), module)
    ...
    def create_database_statistics_panel(
        self, stats: Optional[List[DatabaseStats]] = None
    ) -> Panel:
    ...
    def create_module_menu_panel(
        self, module_name: str, selected_index: int = 0
    ) -> Panel:
        ...
        for i, (item, description) in enumerate(zip(menu_items, descriptions)):
            if i == selected_index:
                ...
                menu_content.append(Text(f"   {description}", style="dim white"))
    ...
    def create_symbol_explorer_panel(
        self, symbols_data: Optional[Dict] = None
    ) -> Panel:
        ...
        for symbol, data in symbols_data.items():
            symbol_node = tree.add(
                f"[{ColorScheme.SYMBOL}]{symbol}[/] ({data.get('exchange', 'Unknown')})"
            )
...
class LayoutManager:
    ...
    def _setup_layout(self) -> None:
        ...
        if self.layout_type == LayoutType.THREE_PANEL:
            ...
            self.layout["body"].split_row(
                Layout(name="menu", ratio=1), Layout(name="content", ratio=2)
            )
            ...
            self.layout.split(Layout(name="content", ratio=1))
    ...
    def get_panel_names(self) -> List[str]:
        ...
        def collect_names(layout_item: Any, names: List[str]) -> None:
