...
class TUIDiagnostics:
    ...
    def test_imports(self):
        ...
        try:
            ...
            self.log_result("Main TUI Import", "PASS", "Successfully imported RithmicAdminTUI")
        except Exception as e:
            self.log_result("Main TUI Import", "FAIL", f"Failed to import: {str(e)}", traceback.format_exc())
            return False
    ...
    def test_tui_initialization(self):
        ...
        try:
            ...
            if hasattr(app, 'keyboard_handler'):
                self.log_result("Keyboard Handler", "PASS", f"Handler type: {app.keyboard_handler}")
            else:
                self.log_result("Keyboard Handler", "FAIL", "No keyboard_handler attribute")
            
            # Test display manager
            if hasattr(app, 'display_manager'):
                self.log_result("Display Manager", "PASS", "Display manager initialized")
            else:
                self.log_result("Display Manager", "FAIL", "No display_manager attribute")
            
            return app
            
        except Exception as e:
            self.log_result("TUI Initialization", "FAIL", f"Initialization failed: {str(e)}", traceback.format_exc())
            return None
        ...
        for panel_name, method_name in panel_tests:
            try:
                if hasattr(display_manager, method_name):
                    ...
                    if panel:
                        self.log_result(panel_name, "PASS", "Panel rendered successfully")
                        
                        # Check if panel uses Live (TUI live)
                        if hasattr(panel, '__class__') and 'Panel' in str(panel.__class__):
                            self.log_result(f"{panel_name} Live Check", "PASS", "Uses Rich Panel (compatible with Live)")
                    else:
                        self.log_result(panel_name, "FAIL", "Panel method returned None")
                else:
                    self.log_result(panel_name, "FAIL", f"Method {method_name} not found")
                    
            except Exception as e:
                self.log_result(panel_name, "FAIL", f"Panel rendering failed: {str(e)}", traceback.format_exc())
    ...
    def test_layout_rendering(self, app):
        ...
        try:
            ...
            if layout:
                self.log_result("Complete Layout", "PASS", "Layout rendered successfully")
                
                # Check if it's a Rich Layout (compatible with Live)
                if hasattr(layout, '__class__') and 'Layout' in str(layout.__class__):
                    self.log_result("Layout Live Compatibility", "PASS", "Uses Rich Layout (Live compatible)")
                else:
                    self.log_result("Layout Live Compatibility", "WARN", f"Unknown layout type: {type(layout)}")
            else:
                self.log_result("Complete Layout", "FAIL", "Layout rendering returned None")
                
        except Exception as e:
            self.log_result("Complete Layout", "FAIL", f"Layout rendering failed: {str(e)}", traceback.format_exc())
        ...
        try:
            ...
            if hasattr(app.display_manager, 'start_live_display'):
                ...
                if hasattr(app.display_manager, 'update_live_display'):
                    ...
                    self.log_result("Update Live Display", "PASS", "Live display updated")
                else:
                    self.log_result("Update Live Display", "FAIL", "update_live_display method missing")
                ...
                if hasattr(app.display_manager, 'stop_live_display'):
                    ...
                    self.log_result("Stop Live Display", "FAIL", "stop_live_display method missing")
                    
            else:
                self.log_result("Live Display Methods", "FAIL", "start_live_display method missing")
                
        except Exception as e:
            self.log_result("Live Display Test", "FAIL", f"Live display test failed: {str(e)}", traceback.format_exc())
    ...
    def test_keyboard_navigation(self, app):
        ...
        try:
            ...
            if hasattr(app, 'get_key_input'):
                ...
                self.log_result("Key Input Method", "PASS", f"Method exists, returned: {result}")
            else:
                self.log_result("Key Input Method", "FAIL", "get_key_input method missing")
            
            # Test process_key method
            if hasattr(app, 'process_key'):
                self.log_result("Process Key Method", "PASS", "process_key method exists")
            else:
                self.log_result("Process Key Method", "FAIL", "process_key method missing")
                
            # Test keyboard handler setup
            keyboard_handler = getattr(app, 'keyboard_handler', None)
            if keyboard_handler:
                self.log_result("Keyboard Handler Setup", "PASS", f"Handler: {keyboard_handler}")
            else:
                self.log_result("Keyboard Handler Setup", "WARN", "No keyboard handler detected")
                
        except Exception as e:
            self.log_result("Keyboard Navigation", "FAIL", f"Keyboard test failed: {str(e)}", traceback.format_exc())
    ...
    async def test_async_operations(self, app):
        ...
        try:
            ...
            if hasattr(app, 'process_key'):
                ...
                self.log_result("Async Key Processing", "PASS", "process_key handled async call")
            else:
                self.log_result("Async Key Processing", "FAIL", "process_key method missing")
                
        except Exception as e:
            self.log_result("Async Operations", "FAIL", f"Async test failed: {str(e)}", traceback.format_exc())
    ...
    def generate_report(self):
        ...
        print("\n🖥️  TUI LIVE ANALYSIS:")
        print("-" * 30)
        live_compatible_panels = [r for r in self.results if "Live" in r['test'] and r['status'] == 'PASS']
        ...
        print("-" * 40)
        keyboard_tests = [r for r in self.results if "Key" in r['test'] or "Keyboard" in r['test']]
