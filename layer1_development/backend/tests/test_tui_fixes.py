...
def test_callback_signature():
    ...
    try:
        ...
        test_cases = [
            ...
            (
                ("Message", 0.25, "extra", "fourth_arg"),
                {},
                "Four arguments (the failing case)",
            ),
            ((), {"message": "Keyword message"}, "Keyword message"),
            (
                (),
                {"message": "Keyword message", "progress": 0.8},
                "Keyword message + progress",
            ),
        ...
        for message, status_type in status_tests:
            try:
                ...
                if app.status.last_operation_result == expected:
                    ...
                    print(
                        f"❌ {status_type.upper()} status update - FAILED: Expected '{expected}', got '{app.status.last_operation_result}'"
                    )
                    return False
            except Exception as e:
        ...
        for message in test_messages:
            try:
                ...
                if status_panel:
                    ...
                    print(
                        f"❌ Status panel for '{message[:20]}...' - FAILED: No panel returned"
                    )
                    return False
